...
public class RateIt {
    ...
    static boolean isProxyEnabled() {
        ServiceTracker proxyTracker = new ServiceTracker(PresentationPlugin.getDefault()
    ...
    static boolean rate(int value, String comment) {
        ...
        try {
            ...
            String content = "rating=" + value + "&ratinglid=559&ratinguser=outside"
            + "&req=addrating";
            ...
            PresentationPlugin.errorDialog("Failed to rate on EPIC...", e);
            return false;
        } finally {
            if (out != null) {
                try {
                    ...
                    PresentationPlugin.errorDialog("Failed to close stream...", e);
                }
            }
            if (input != null) {
                try {
    ...
    static void createTextArea(Composite defPanel) {
        ...
        if (ratingDone) {
            ...
            .setText("Unfortunately,\nhttp proxy "
                    + "is enabled and therefore you can't rate Extended VS Presentation plugin now...\n"
                    + "Please try to rate for Extended VS Presentation plugin\nfrom another workstation,\nuse this url:\n"
                    + "www.eclipseplugincentral.com/Web_Links-index-req-ratelink-lid-559-ttitle-Extended_VS_Presentation.html");
            combo.setEnabled(false);
            textArea.setEditable(false);
            okButton.setEnabled(false);
        }
